...
if ( !class_exists( 'THE7_RWMB_Datetime_Field' ) )
{
	class THE7_RWMB_Datetime_Field
	{
		/**
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_register_script( 'jquery-ui-timepicker', THE7_RWMB_JS_URL . 'jqueryui/jquery-ui-timepicker-addon.js', array( 'jquery-ui-datepicker', 'jquery-ui-slider' ), '0.9.7', true );
			...
			if ( file_exists( THE7_RWMB_DIR . 'js/' . $date_path ) )
			{
				wp_register_script( 'jquery-ui-datepicker-i18n', THE7_RWMB_JS_URL . $date_path, array( 'jquery-ui-datepicker' ), '1.8.17', true );
				$deps[] = 'jquery-ui-datepicker-i18n';
			}
			if ( file_exists( THE7_RWMB_DIR . 'js/' . $time_path ) )
			{
				wp_register_script( 'jquery-ui-timepicker-i18n', THE7_RWMB_JS_URL . $time_path, array( 'jquery-ui-timepicker' ), '1.8.17', true );
				$deps[] = 'jquery-ui-timepicker-i18n';
			}

			wp_enqueue_script( 'the7-mb-datetime', THE7_RWMB_JS_URL . 'datetime.js', $deps, THE7_RWMB_VER, true );
			wp_localize_script( 'the7-mb-datetime', 'THE7_RWMB_Datetimepicker', array( 'lang' => $locale ) );
		}
		...
		{
			return sprintf(
				'<input type="text" class="the7-mb-datetime" name="%s" value="%s" id="%s" size="%s" data-options="%s">',
				$field['field_name'],
